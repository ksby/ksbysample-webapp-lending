...
@SpringBootTest
class FreeMarkerHelperTest extends Specification {
    ...
    def "テンプレートファイルから文字列を生成する_変数のみの場合"() {
        ...
        expect:
        freeMarkerHelper.merge("mail/FreeMarkerUtilsTest-001.ftl", model) == "田中　太郎"
    }
    ...
    def "テンプレートファイルから文字列を生成する_変数がnullの場合"() {
        ...
        expect:
        freeMarkerHelper.merge("mail/FreeMarkerUtilsTest-001.ftl", model) == ""
    }
    ...
    def "テンプレートファイルから文字列を生成する_変数＋クラスの場合"() {
        ...
        expect:
        freeMarkerHelper.merge("mail/FreeMarkerUtilsTest-002.ftl", model) == result
    }
    ...
    def "テンプレートファイルから文字列を生成する_リストの場合"() {
        ...
        expect:
        freeMarkerHelper.merge("mail/FreeMarkerUtilsTest-003.ftl", model) == result
    }
    ...
    def "テンプレートファイルが存在しない場合はエラーになる"() {
        ...
        when:
        freeMarkerHelper.merge("mail/FreeMarkerUtilsTest-notFound.ftl", model)
